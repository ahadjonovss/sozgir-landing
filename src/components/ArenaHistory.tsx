 ...
 *  qanday tugagan va qancha o'lja olingan. O'yinlarni ajratmaydi —
        ...
        <ul className="history">
          {rows.map((row) => (
            <li key={row.id} className={`history__row history__row--${row.outcome}`}>
              ...
              {row.oljaDelta !== null && row.oljaDelta !== 0 && (
                <span className="history__delta" title="Jangda olingan o‘lja">
                  <OljaDelta value={row.oljaDelta} />
                </span>
              )}
            </li>
          ))}
